...
const MermaidPlugin = (md, options) => {
  ...
  md.renderer.rules.fence = (tokens, idx, ops, env, slf) => {
    ...
    if (token.info === "mermaid") {
      ...
      return `<div class="${config.prefix}-mermaid" ${line !== void 0 ? "data-line=" + line : ""} data-mermaid-theme=${options.themeRef.value}">${md.utils.escapeHtml(code)}</div>`;
    }
...
const useMarkdownIt = (props, previewOnly) => {
  const { editorConfig, markdownItConfig, markdownItPlugins, editorExtensions } = config.configOption;
  const editorId = vue.inject("editorId");
      ...
      replaceMermaid().then(() => {
        ...
        if ((_a = editorExtensions.mermaid) == null ? void 0 : _a.enableZoom) {
          ...
          clearMermaidEvents = dom.zoomMermaid(
            (_b = rootRef.value) == null ? void 0 : _b.querySelectorAll(`#${editorId} p.${config.prefix}-mermaid`),
            {
